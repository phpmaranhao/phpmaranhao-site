...
class Wpr_upgrader {
	...
	function Wpr_upgrader($skin = null) {
		return $this->__construct($skin);
	}
	function __construct($skin = null) {
		if ( null == $skin )
			$this->skin = new Wpr_upgrader_Skin();
		else
			$this->skin = $skin;
	}
...
//			$this->skin->feedback('maintenance_end');
        ...
        function wpr_delete_transient($option_name) {
                if(trim($option_name) == ''){
                    return FALSE;
                }
 ...
 * @since 2.8.0
 */
class Wpr_Plugin_Upgrader extends Wpr_upgrader {
	...
	function install($package) {
		...
		$this->wpr_delete_transient('update_plugins');
 ...
 * @since 2.8.0
 */
class Wpr_Theme_Upgrader extends Wpr_upgrader {
	...
	function install($package) {
		...
		$this->wpr_delete_transient('update_themes');
	...
	function upgrade($theme) {
		...
		$current = $this->wpr_get_transient( 'update_themes' );
 ...
 * @since 2.8.0
 */
class Wpr_Core_Upgrader extends Wpr_upgrader {
 ...
 * @since 2.8.0
 */
class Wpr_upgrader_Skin {

	var $upgrader;
	var $done_header = false;
	var $result = false;

	function Wpr_upgrader_Skin($args = array()) {
 ...
 * @since 2.8.0
 */
class Wpr_Plugin_Upgrader_Skin extends Wpr_upgrader_Skin {
	var $plugin = '';
	var $plugin_active = false;
	var $plugin_network_active = false;

	function Wpr_Plugin_Upgrader_Skin($args = array()) {
		return $this->__construct($args);
	}
 ...
 * @since 3.0.0
 */
class Wpr_Bulk_Upgrader_Skin extends Wpr_upgrader_Skin {
	var $in_loop = false;
	var $error = false;
...
class Wpr_Bulk_Plugin_Upgrader_Skin extends Wpr_Bulk_Upgrader_Skin {
	var $plugin_info = array(); // Wpr_Plugin_Upgrader::bulk() will fill this in.
	function Wpr_Plugin_Upgrader_Skin($args = array()) {
		parent::__construct($args);
	}
...
class Wpr_Bulk_Theme_Upgrader_Skin extends Wpr_Bulk_Upgrader_Skin {
	var $theme_info = array(); // Wpr_Theme_Upgrader::bulk() will fill this in.
	function Wpr_Theme_Upgrader_Skin($args = array()) {
		parent::__construct($args);
	}
 ...
 * @since 2.8.0
 */
class Wpr_Plugin_Installer_Skin extends Wpr_upgrader_Skin {
	var $api;
	var $type;
 ...
 * @since 2.8.0
 */
class Wpr_Theme_Installer_Skin extends Wpr_upgrader_Skin {
	var $api;
	var $type;
 ...
 * @since 2.8.0
 */
class Wpr_Theme_Upgrader_Skin extends Wpr_upgrader_Skin {
	var $theme = '';
 ...
 * @since 2.8.0
 */
class Wpr_File_Upload_Upgrader {
	var $package;
	var $filename;
